...
- **技术架构方案 v1.1**：路径 `ontology-demo/方案/项目风险管控工具-技术架构方案-v1.1.md`。2026-08-25 修复了 4 个 Mermaid 图预览错误（1.1/1.2/3.1/6.1），根因：① `@xyflow/react` 中 `@` 在节点标签起始或 `<br/>` 后被解析器误判为指令，修复方式 `P3["...@xyflow/react"]` 加双引号包裹；② 6.1 节点标签 `[]` 内嵌双引号 `"` 导致解析中断，修复方式去掉内层引号并整体加双引号包裹。还移除了 1.1 中 subgraph 内冗余的 `direction TB`（旧版 Mermaid 不支持）。**Mermaid 经验**：节点标签含 `@`、`"`、`(` 等特殊字符时，一律用双引号包裹 `ID["text"]` 最安全。
