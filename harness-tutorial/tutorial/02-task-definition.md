...
> **一句话概括**：Task 就是给 Agent 的「标准化考试卷」——相同的题目、相同的评分标准，才能公平地比较每次表现。

---

## 通俗理解

上一章说手动测试就像“每次出不同的题、用不同的标准评分”。这一章我们来解决这个问题——把任务变成一份**标准化的考试卷**。

一份好试卷需要什么？

- **题号**（`id`）：知道这是哪道题
- **题目**（`input`）：明确要做什么
- **评分标准**（`expected`）：怎么算对、怎么算错
- **答题要求**（`constraints`）：字数限制、格式要求

有了这些，不管谁来答题、答多少次，评分标准都一样。
...
> **类比**：把 Task 想象成一张考试卡片。正面写题目（`input`），背面写答案（`expected`），边上还印着答题规则（`constraints`）。
...
> **反面教材**：什么叫“好”？什么叫“更好”？这种模糊要求，就像让老师批作文时只写“好”或“不好”，学生根本不知道哪里可以改进。
...
本章你做了第一份「标准化试卷」——用 YAML 定义了 3 个任务，每个都有明确的输入、评分标准和约束条件。

下一章我们来做「考生」——实现一个 Agent Runner，让它能接收任务、给出答案。

## 检查点

- [ ] 能用「考试卷」的类比解释 Task 的五个字段
