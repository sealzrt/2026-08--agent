...
> **一句话概括**：Tool Registry 就是给考生配一个「工具箱」——计算器、文本检索这些工具统一注册到一个地方，按名字就能拿出来用。

---

## 通俗理解

前几章我们有了「试卷」（Task）和「考生」（Agent Runner）。但有些题目光靠脑子做不够，还需要用计算器、查字典。

Tool Registry 就是这个「工具箱」：

| 日常类比 | 代码中的对应 |
|---------|----------------|
| 工具箱里的螺丝刀、扳手 | `calculator`、`text_search` 等工具 |
| 工具箱的收纳盒 | `ToolRegistry` 注册表 |
| 用完放回去、记录损坏 | 每次调用记录输入、输出、错误 |

关键是：**工具失败时不能把整个考场炸了**，而是要记录“哪个工具坏了、什么原因”，然后继续考下一题。
...
> **类比**：就像 USB 接口——不管是键盘还是鼠标，插上去就能用，因为大家都遵循同一个接口标准。工具协议也一样，每个工具都用统一的 `{name, input, output, error}` 格式。
...
> **类比**：工具失败就像你用计算器算“1÷0”，计算器显示“错误”而不是爆炸。我们的工具也应该优雅地返回错误信息。
...
本章你给考生配了一个「工具箱」——Tool Registry，注册了计算器和文本检索两个工具，并统一了工具的输入输出协议。

现在的问题是：考生答完了题，但没有人记录答题过程。下一章我们来做「监考员」——Trace Logger，记录每次运行的完整过程。

## 检查点

- [ ] 能用「工具箱」的类比解释 Tool Registry
