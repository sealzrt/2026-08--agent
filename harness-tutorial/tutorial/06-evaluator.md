...
> **一句话概括**：Evaluator 就是「阅卷老师」——拿着评分标准（expected）对照考生的答案，给出通过/不通过的判断和具体原因。

---

## 通俗理解

前面的章节我们有了试卷（Task）、考生（Agent Runner）、工具箱（Tool Registry）和监控录像（Trace）。

但考试结束后，总得有人来批卷。Evaluator 就是这个「阅卷老师」。

阅卷老师怎么批卷？

1. **拿出评分标准**：从 Task 的 `expected` 中读取“必须包含的关键词”
2. **对照答案**：检查考生的 `answer` 是否满足每条标准
3. **写出扣分原因**：不是简单地给个“不及格”，而是说清楚“缺少关键词 A”“超过字数限制”

这样做的好处是：下次考生看到评语就知道哪里该改进，而不是只看到一个“F”。
...
> **类比**：第一版只实现两种最简单的评分规则，就像老师批试卷只检查两个东西：
> - 有没有出现关键词（`must_include`）——“有没有答到要点”
> - 字数有没有超标（`max_words`）——“有没有写太多”
...
> **类比**：规则评估就像选择题自动阅卷——快速、稳定、可复现。模型评估就像作文阅卷——能判断逻辑、语气、连贯性，但成本高、结果不稳定。
>
> 入门阶段先把「选择题自动阅卷」做好，以后再考虑「作文阅卷」。
...
本章你做了一个「阅卷老师」——Evaluator，能根据评分标准自动判断答案是否通过，并给出具体的失败原因。

现在每个任务都有了成绩（通过/不通过）。但成绩散在各处，没有一个地方能一眼看到所有人的表现。下一章我们来做「成绩单」——Report。

## 检查点

- [ ] 能用「阅卷老师」的类比解释 Evaluator
