...
> **一句话概括**：Agent Runner 就是「考生」——它接收试卷（Task），给出答案（结构化结果），不管答对答错都填写答题卡。

---

## 通俗理解

上一章我们做了「试卷」（Task）。这一章来做「考生」。

一个好考生需要做到什么？

1. **接收试卷**：拿到 Task 中的题目
2. **认真答题**：调用模型或模拟逻辑，生成答案
3. **规范填写答题卡**：返回结构化结果，不是随手写在草稿纸上
4. **遇到问题也要报告**：比如“题目看不懂”、“时间到了”，而不是直接交白卷

最重要的是第 4 点——考生即使做不出来，也要在答题卡上写清楚“哪道题做不出来，原因是什么”，而不是让监考老师去猜。
...
> **类比**：考生只负责答题，不负责出题、批卷、写报告。出题是 Task Loader 的事，批卷是 Evaluator 的事，写报告是 Report Writer 的事。
...
> **类比**：Agent Runner 的接口就像一家餐厅的外卖窗口——顾客（调用者）只需要知道“给订单、拿餐”，不需要知道厨房里怎么做。
...
本章你做了一个「考生」——Agent Runner，它能接收任务并返回结构化结果。现在的状态是：有试卷、有考生，但考生还没有工具可以用。

下一章我们给考生配一个「工具箱」——Tool Registry。

## 检查点

- [ ] 能用「考生答题」的类比解释 Agent Runner
