...
1. 一句话概括（blockquote）
2. 图解（PNG 嵌入）
3. 本章你会学到什么（3-5 个学习目标）
4. 通俗理解（类比 + 日常例子）
5. 先记住这几件事（初学者优先掌握的关键点）
6. 核心概念（简短定义）
7. 本章新增机制（只解释相对上一章新增的部分）
8. 代码（完整可运行 + 详细注释）
9. 代码拆解（按执行顺序解释）
10. 运行它（一条命令 + 预期现象）
11. 常见卡点（环境、路径、API、权限、输出）
12. 动手试一试（1-2 个练习）
13. 小结与下一章

## 内容优化规范

- 每章只强调一个新机制，避免同时引入多个抽象。
- 先解释“为什么需要它”，再解释“代码怎么写”。
- 代码前先给读者一个阅读入口：本章新增函数、常量或分支在哪里。
- 代码后必须说明运行后应该看到什么，避免读者不知道成功标准。
- 常见卡点要写成可操作检查项，例如“确认当前目录是 `harness-visual-tutorial`”。
- 图片说明不重复图片标题，而是解释读图顺序和图里的关键关系。
