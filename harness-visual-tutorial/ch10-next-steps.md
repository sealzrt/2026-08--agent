...
- 完成最小 Harness 后，常见扩展方向有哪些
- 每条路线解决什么问题
- 应该根据自己的目标先学哪一条
- 每条路线可以从哪个小练习开始

这一章不是要求你马上全都学，而是帮你选择下一步。
...
| 你的目标 | 建议路线 | 原因 |
|----------|----------|------|
| 想连接数据库、浏览器、第三方 API | 路线 1：MCP 插件 | 先扩大工具能力 |
| 想处理复杂任务、多人协作式分析 | 路线 2：多 Agent 团队 | 先学任务拆分和协作 |
| 想让 Agent 定时跑、后台跑 | 路线 3：任务调度 | 先学运行管理 |
| 想把固定流程自动化 | 路线 4：工作流引擎 | 先学流程编排 |

建议一次只选一条。不要同时上 MCP、多 Agent、调度和工作流，否则很难判断问题出在哪里。
...
先不要接真实数据库。可以写一个“假 MCP 工具”：

```python
def fake_mcp_weather(city: str) -> str:
    return f"{city} 今天晴，适合测试 Harness。"
```

把它当作新工具注册进 `TOOLS` 和 `TOOL_HANDLERS`。等工具注册流程熟了，再换成真实 MCP Server。
...
把 ch06 的 `delegate` 改成两个固定子代理：

```text
Research Agent：只负责读取资料并总结事实
Review Agent：只负责检查总结是否遗漏风险
```

主 Agent 先调用 Research Agent，再把结果交给 Review Agent。这样你能先理解“流水线”，不用一开始就做复杂协作。
...
while True:
    print("每 60 秒检查一次项目状态")
    time.sleep(60)
```

再把 `print` 换成一次 Agent 调用。这样你能先理解“谁触发 Agent 运行”。
...
```text
读取文件 → 让 LLM 总结 → 把总结打印出来
```

确认流程能跑后，再把步骤改成配置文件。不要一开始就引入复杂引擎。
...
完成本教程后，不要求你能写出工业级 Agent 框架，但应该能做到：

- 看懂一个 Agent Harness 的主循环
- 知道工具调用需要定义、实现、注册、分发
- 知道工具执行前必须做权限检查
- 知道长期记忆和上下文管理解决的是不同问题
- 知道子代理本质上是嵌套的独立 Agent Loop
- 知道错误处理要把失败转成可继续处理的上下文
- 能从零写一个小型、可运行、可扩展的 Agent Harness
