...
| C6 审计追溯 | semantica、jonex | 决策记录五要素（谁/何时/依据/决策/结果）；状态快照可回放；答案带引用段落 | 模式借鉴（自研） |
...
```mermaid
flowchart TB
    UI[Web 界面<br/>React + AntD]
    
    subgraph API[后端层（FastAPI 一体化）]
        C1[C1 本体建模工作台]
        C7[C7 一致性/偏差检测]
        C6[C6 审计追溯]
        C5[C5 控制平面闸门]
...
2. **后端 8 个是 Python 生态**（ontogpt/ontocast/semantica/trustgraph/jonex/fusion-jena/ontopilot/OpenCrab），LLM 抽取、规则推理、文档解析的成熟组件全在 Python 侧
...
sequenceDiagram
    ...
    participant E as FastAPI 后端<br/>(业务API+识别引擎一体)
...
**做什么**：每个风险/决策都有完整"决策链"，可回放、可追溯。这是对合同 8.1"项目管理可审计"的产品化落地。
...
| 识别引擎 | 无（规则内嵌） | FastAPI 内置模块（ontogpt 直接 import） | 8/10 参考项目是 Python 生态；免服务间 REST |
...
### 8.2 后端 API 清单（FastAPI 一体化服务对外）

| 接口 | 方法 | 功能 | 调用方 |
|---|---|---|---|
| `/api/parse` | POST | 文档 → 带编号文本块 | 前端（C2） |
| `/api/extract` | POST | 文本块 → 要素候选（ontogpt） | 前端（C3） |
| `/api/rules/run` | POST | 要素集 → 规则推理结果 | 前端（C4） |
| `/api/rules/validate` | POST | 规则 DSL 语法校验 | 前端（规则编辑器） |
| `/api/reconcile` | POST | 跨源对账（C7） | 前端 |
| `/api/health` | GET | 健康检查 | 运维 |

> LLM 默认关闭：未配置 API Key 时，`/api/extract` 降级为"规则式抽取"（关键词+正则），保证 MVP 开箱即用。
