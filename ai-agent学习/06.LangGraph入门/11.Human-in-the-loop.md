...
- `interrupt(...)` 是**节点代码里的函数调用**。它的作用是告诉 LangGraph：这里该暂停了。
- `__interrupt__` 里的 `Interrupt(...)` 是**`graph.invoke()` 的返回结果里携带的对象表示**。它不是再次调用 `interrupt()`，而是运行时把暂停信息包装成结果返回给调用方。

- `interrupt({...})` 会**暂停图执行**，把它参数里的数据返回给调用方（前端可以拿它渲染审批卡片）。
- 调用方看到 `__interrupt__` 字段，就知道"图在等人"。
- 这一行第一次执行时，不会继续往下跑，图会停在这里。
- 恢复时，这一行会重新执行，`feedback` 才会变成人工传回来的内容。
- 传给 `interrupt()` 的数据，通常是"给人看的上下文 + 要确认的问题"；返回给调用方时，会被放进 `__interrupt__`。
- 这里的 `feedback` 变量名只是占位，第一次调用时它还拿不到人工反馈，真正的反馈是在 `Command(resume=...)` 恢复后才传回来。
...
第一次 `invoke()` 触发 `interrupt()` 后，**返回的是 `invoke()` 的结果，不是 `interrupt()` 的返回值**。这个结果通常包含：

1. 已经算出来的完整状态。
2. `__interrupt__` 列表，里面是等待人工处理的数据。
