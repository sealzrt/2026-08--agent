...
```mermaid
sequenceDiagram
    participant App as 调用方(前端/服务)
    participant G as 图(compile(checkpointer))
...
1. `write_draft` 执行，`draft_report` 生成。
2. `human_review` 执行到 `interrupt(...)`，暂停。
3. 图返回完整状态 + `__interrupt__`（里面是草稿和问题）。
4. checkpointer 已保存当前状态。

**调用方响应：**

- 前端展示 `__interrupt__.value`（草稿 + 问题）。
- 人工点击"驳回并补充意见"，产生反馈 `{"approved": False, "comment": "补充风险说明"}`。

**第二次 `invoke`（`Command(resume=...)`）：**

1. 用同一 `thread_id` 找到断点。
2. `human_review` **从头重新运行**。
3. `interrupt()` 返回人工反馈 `{"approved": False, "comment": "补充风险说明"}`。
4. `feedback.get("approved")` 为 False，走"驳回"分支。
5. 返回 `final_report = 草稿 + 修改意见`。
