...
# 审核人提交审批意见后：API / 脚本再次调用，恢复同一个 thread
result = graph.invoke(Command(resume=feedback), config)
```

普通用户通常不直接调用 `compile()` 或 `invoke()`：普通用户在前端填写主题、点击提交；审核人在审批界面给出批准或驳回意见；后端代码负责把这些操作转换成 `invoke()` 调用。也就是说：

- `compile()` 是**组装应用**，不是执行任务。
- `invoke()` 是**执行任务**，不是编译图。
- `interrupt()` 是图在执行过程中把控制权交回后端，后端等待审核人反馈。

### HITL 完整时序

这里把交互角色拆成两类：**普通用户**负责发起任务和查看结果，**审核人**负责在中断后给出批准或驳回意见。
