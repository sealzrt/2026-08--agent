...
流程：API 只负责**接收任务、返回 task_id**；真正的图执行交给后台 Worker；前端通过 SSE 订阅事件（第 12 章），通过 API 查状态。
...
1. **`POST /tasks` 立即返回 `task_id`**，不等待图执行完——长任务绝不阻塞 HTTP。
...
GET  /tasks/{task_id}/events     订阅事件流（SSE）
