...
**排查**：本教程第 01-13 章都有 `code/step_by_step/` 单文件示例，便于逐章运行；第 07 章起同时开始引用 `code/langgraph_intro_project/` 项目结构。运行项目模块时，在项目根目录使用 `python -m app.xxx`；运行单文件示例时，进入 `code/step_by_step` 后直接 `python xx.py`。
