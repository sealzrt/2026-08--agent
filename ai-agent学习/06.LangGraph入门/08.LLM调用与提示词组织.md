...
```mermaid
flowchart LR
    RAW["模型返回<br/>可能是任意文本"] -->|"第一步：开 JSON mode<br/>降低失败率"| CLEAN["清洗<br/>去 Markdown 包裹"]
    CLEAN -->|"第二步：json.loads"| TRY{"能解析？"}
    TRY -- "能" --> MISS{"缺字段 / 类型不对？"}
...
把 Prompt 放到独立文件 `app/prompts/xxx.md`，代码里加载并格式化：
...
```python
MODE = os.getenv("AGENT_MODE", "mock")   # "mock" 或 "real"
```
...
```mermaid
flowchart LR
    CFG["config<br/>AGENT_MODE / USE_MOCK"] --> SW{"选择模式"}
...
3. **切换开关**：`AGENT_MODE` 配置从 `mock` 改成 `real`。
