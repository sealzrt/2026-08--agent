...
教程版 API 以输入输出转换为主，但当前项目会同步调用一次图，便于把完整流程讲清楚：
...
API 不拼 Prompt、不定图；真实生产里图执行应下沉到后台 Worker。
...
本章的"改造"就是**把前面 13 章的能力放进正确位置**。对照清单：
...
| API 层拼 Prompt / 调模型 | 分层混乱 | API 层主要做输入输出转换，调用图 |
